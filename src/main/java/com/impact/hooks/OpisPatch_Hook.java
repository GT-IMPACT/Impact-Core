...
public class OpisPatch_Hook {
  ...
  public static ArrayList<DataBlockTileEntityPerClass> getCumulativeTimingTileEntities(
      TileEntityManager ignored) { //shitty decompile but it works
    ...
    for (
        Iterator<CoordinatesBlock> iterator = ((ProfilerTileEntityUpdate) ProfilerSection.TILEENT_UPDATETIME
            .getProfiler()).data.keySet().iterator(); iterator.hasNext(); data.get(id, meta).add(
        ((ProfilerTileEntityUpdate) ProfilerSection.TILEENT_UPDATETIME.getProfiler()).data
            .get(coord).getGeometricMean())) {
      coord = iterator.next();
      World world = DimensionManager.getWorld(coord.dim);
      id = Block.getIdFromBlock(block = world.getBlock(coord.x, coord.y, coord.z));
      if (block instanceof GT_Block_Machines && (tileEntity = world
          .getTileEntity(coord.x, coord.y, coord.z)) instanceof IGregTechTileEntity) {
