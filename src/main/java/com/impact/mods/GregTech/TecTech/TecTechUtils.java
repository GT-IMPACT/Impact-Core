...
public class TecTechUtils {
  ...
  public static boolean addEnergyInputToMachineList(ITecTechEnabledMulti baseTE,
      IGregTechTileEntity te, int aBaseCasingIndex) {
  ...
  public static boolean drainEnergyMEBFTecTech(ITecTechEnabledMulti multi, long aEU) {
    ...
    return hasDrained && (multi.getVanillaEnergyHatches().size() > 0
        || multi.getTecTechEnergyTunnels().size() > 0 || multi.getTecTechEnergyMultis().size() > 0);
  }
