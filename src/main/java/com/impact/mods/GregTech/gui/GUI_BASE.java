  ...
  public static class GT_RectHandler
    ...
    public List<String> handleTooltip(GuiContainer gui, int mousex, int mousey,
      ...
      if ((canHandle(gui)) && (currenttip.isEmpty())) {
        if (gui instanceof GUI_BASE && new Rectangle(148, 52, 17, 17).contains(new Point(
            GuiDraw.getMousePosition().x - ((GUI_BASE) gui).getLeft()
                - codechicken.nei.recipe.RecipeInfo.getGuiOffset(gui)[0],
            GuiDraw.getMousePosition().y - ((GUI_BASE) gui).getTop()
    ...
    private boolean transferRect(GuiContainer gui, boolean usage) {
      if (gui instanceof GUI_BASE) {
        return (canHandle(gui)) && (new Rectangle(148, 52, 17, 17).contains(new Point(
            GuiDraw.getMousePosition().x - ((GUI_BASE) gui).getLeft()
                - codechicken.nei.recipe.RecipeInfo.getGuiOffset(gui)[0],
            GuiDraw.getMousePosition().y - ((GUI_BASE) gui).getTop()
                - codechicken.nei.recipe.RecipeInfo.getGuiOffset(gui)[1]))) && (usage
            ? GuiUsageRecipe.openRecipeGui(((GUI_BASE) gui).mNEI, new Object[0])
            : GuiCraftingRecipe.openRecipeGui(((GUI_BASE) gui).mNEI, new Object[0]));
      }
      return false;
    }
