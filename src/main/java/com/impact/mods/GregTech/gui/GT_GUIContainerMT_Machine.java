...
public class GT_GUIContainerMT_Machine extends GT_GUIContainer {
    ...
    public GT_GUIContainerMT_Machine(InventoryPlayer aInventoryPlayer, IGregTechTileEntity aTileEntity, String aGUIbackground) {
    ...
    public void renderText(List strings, int x, int y, FontRenderer font) {
        if (!strings.isEmpty()) {
            ...
            if (strings.size() > 1) i1 += 2 + (strings.size() - 1) * 10;
            if (x2 + k > this.width) x2 -= 28 + k;
            if (y2 + i1 + 6 > this.height) y2 = this.height - i1 - 6;
