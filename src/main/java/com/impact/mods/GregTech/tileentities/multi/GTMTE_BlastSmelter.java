...
public class GTMTE_BlastSmelter extends GT_MetaTileEntity_MultiBlockBase {
  ...
  public ITexture[] getTexture(IGregTechTileEntity aBaseMetaTileEntity, byte aSide, byte aFacing,
    ...
    if (aSide == aFacing) {
      return new ITexture[]{Textures.BlockIcons.casingTexturePages[1][51], new GT_RenderedTexture(
  ...
  public boolean checkRecipe(ItemStack aStack) {
    for (GT_MetaTileEntity_Hatch_InputBus tBus : mInputBusses) {
      ...
      if (tInputList.size() > 0) {
        ...
        GT_Recipe tRecipe = GT_Recipe.GT_Recipe_Map.sBlastSmelterRecipes
            .findRecipe(getBaseMetaTileEntity(), false, gregtech.api.enums.GT_Values.V[tTier],
                tFluids, tInputs);
        if ((tRecipe != null) && (this.mHeatingCapacity >= tRecipe.mSpecialValue) && (tRecipe
          ...
          byte overclockCount = calculateOverclockednessEBF(tRecipe.mEUt, tRecipe.mDuration,
              tVoltage);
          ...
          if (tHeatCapacityDivTiers > 0) {
            ...
            this.mMaxProgresstime >>= Math.min(tHeatCapacityDivTiers / 2,
                overclockCount);//extra free overclocking if possible
  ...
  public String[] getInfoData() {
    ...
    return new String[]{
        ...
        StatCollector.translateToLocal("GT5U.multiblock.energy") + ": " + EnumChatFormatting.GREEN
            + Long.toString(storedEnergy) + EnumChatFormatting.RESET + " EU / " +
        ...
        StatCollector.translateToLocal("GT5U.multiblock.problems") + ": " +
            EnumChatFormatting.RED + (getIdealStatus() - getRepairStatus())
            + EnumChatFormatting.RESET +
