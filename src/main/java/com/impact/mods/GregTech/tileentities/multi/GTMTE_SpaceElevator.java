...
public class GTMTE_SpaceElevator extends GT_MetaTileEntity_MultiParallelBlockBase {
    ...
    public void holo() {
        registerMetaClass(GTMTE_SpaceElevator.class, new IMultiblockInfoContainer<GTMTE_SpaceElevator>() {
            //region Structure
        ...
        for(int X = -3; X <= 3; X++) {
            for (byte Y = -1; Y <= 4; Y++) {
                for (byte Z = 3; Z >= -3; Z--) {
                    ...
                    if ((X==-1 || X==0 || X==1) && (Y==1 || Y== 2 || Y == 3)) continue;
                    if ((Z==-1 || Z==0 || Z==1) && (Y==1 || Y== 2 || Y == 3)) continue;

                    if ((X==-1 || X==0 || X==1) && (Z==-1 || Z==0 || Z==1) && Y==4) continue;
    ...
    public void onPostTick(IGregTechTileEntity aBaseMetaTileEntity, long aTick) {
        ...
        if (getBaseMetaTileEntity().isServerSide()) {
            ...
            if (mProgresstime > 20*2) {
                    ...
                    if (tWorld != null) {
                        ...
                        if (tTile instanceof IGregTechTileEntity) {
                            ...
                            if (mte instanceof GTMTE_SpaceElevator) {
                                ((GTMTE_SpaceElevator) mte).mTargetD = getBaseMetaTileEntity().getWorld().provider.dimensionId;
                ...
                if (!((this.mTargetX == getBaseMetaTileEntity().getXCoord()) && (this.mTargetY == getBaseMetaTileEntity().getYCoord()) &&
                        (this.mTargetZ == getBaseMetaTileEntity().getZCoord()) && (this.mTargetD == getBaseMetaTileEntity().getWorld().provider.dimensionId))) {
                    List entities_in_box = getBaseMetaTileEntity().getWorld().getEntitiesWithinAABB(
                            Entity.class, AxisAlignedBB.getBoundingBox(
                                    getBaseMetaTileEntity().getOffsetX(getBaseMetaTileEntity().getFrontFacing(), 2) - 1.5D,
                                    getBaseMetaTileEntity().getOffsetY(getBaseMetaTileEntity().getFrontFacing(), 2) - 2,
                                    getBaseMetaTileEntity().getOffsetZ(getBaseMetaTileEntity().getFrontFacing(), 2) - 1.5D,
                                    getBaseMetaTileEntity().getOffsetX(getBaseMetaTileEntity().getFrontFacing(), 2) + 1.5D,
                                    getBaseMetaTileEntity().getOffsetY(getBaseMetaTileEntity().getFrontFacing(), 2) + 2,
                                    getBaseMetaTileEntity().getOffsetZ(getBaseMetaTileEntity().getFrontFacing(), 2) + 1.5D)
                    );
                    ...
                    for (Object tObject : entities_in_box) {
                        if (((tObject instanceof Entity)) && (!((Entity) tObject).isDead)) {
                            ...
                            if (getBaseMetaTileEntity().isActive()) {
                                ...
                                if ((this.mTargetD == getBaseMetaTileEntity().getWorld().provider.dimensionId) || (!GT_Utility.moveEntityToDimensionAtCoords(tEntity, this.mTargetD, this.mTargetX + 0.5D, this.mTargetY + 0.5D, this.mTargetZ + 0.5D))) {
            ...
            if (getBaseMetaTileEntity().getWorld().provider.dimensionId == 0) {
                ...
                if (tNBT != null) {
                    ...
                    GT_Utility.sendChatToPlayer(aPlayer, "Load Position: X"
                            + this.mTargetX + " Y " + this.mTargetY + " Z " + this.mTargetZ + " Dim " + this.mTargetD);
                } else
