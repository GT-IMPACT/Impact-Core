...
import com.impact.mods.nei.oreplugin.helper.DimensionHelper;
...
public class PluginGT5Base extends PluginBase {
  ...
  protected static String getLocalizedNameForItem(Materials aMaterial, String aFormat) {
    return String.format(aFormat.replace("%s", "%temp").replace("%material", "%s"),
        aMaterial.mDefaultLocalName).replace("%temp", "%s");
  }
  ...
  protected void drawToolTip(String sDimNames) {
    if (toolTips) {
      ttDisplayed = false;
      if (GuiDraw.getMousePosition().y > (int) (Minecraft.getMinecraft().currentScreen.height
          * 0.6f) && GuiDraw.getMousePosition().y < (int) (
          Minecraft.getMinecraft().currentScreen.height * 0.8f)) {
        List<String> dims = DimensionHelper.convertCondensedStringToToolTip(sDimNames);
        int w = calculateMaxW(dims);
        int x = GuiDraw.getMousePosition().x > Minecraft.getMinecraft().currentScreen.width / 2 ?
            this.getGuiWidth() - w - 8 : 0;
        if (dims.size() > 10) {
