...
public class NEI_Impact_MEProvider extends NEI_Impact_Default {
	
	int xPos = 4 + 1;
	int yPos = 8 + 3;
	
	public NEI_Impact_MEProvider(GT_Recipe.GT_Recipe_Map aRecipeMap) {
		super(aRecipeMap);
		this.transferRects.add(
				new RecipeTransferRect(
						new Rectangle(81 - xPos, 40 - yPos, 42, 8),
						getOverlayIdentifier()
				));
	}
	
	public TemplateRecipeHandler newInstance() {
		return new NEI_Impact_MEProvider(this.mRecipeMap);
	}
	...
	public void drawExtras(int aRecipeIndex) {
		...
		if (recipeDesc == null) {
			if (tEUt != 0) {
				...
				drawText(10, 93, trans("154", "Voltage: ") + NumberFormat.getNumberInstance().format(tEUt / this.mRecipeMap.mAmperage)
						+ " EU (" + GT_Values.VN[GT_Utility.getTier(tEUt / this.mRecipeMap.mAmperage)] + ")", -16777216);
			...
			drawText(10, 123, "Stable Particles: " + NumberFormat.getNumberInstance().format(tSpecial), -16777216);
	...
	public class CachedDefaultRecipe extends CachedRecipe {
		...
		public CachedDefaultRecipe(GT_Recipe aRecipe) {
			...
			amXPosInputs = new int[]{15, 33, 51, 15, 33, 51, 15, 33, 51};
			amYPosInputs = new int[]{16, 16, 16, 34, 34, 34, 52, 52, 52};
