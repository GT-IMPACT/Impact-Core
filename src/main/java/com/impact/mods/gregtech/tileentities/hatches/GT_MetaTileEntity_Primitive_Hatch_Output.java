...
public class GT_MetaTileEntity_Primitive_Hatch_Output extends GT_MetaTileEntity_Hatch {
  ...
  public GT_MetaTileEntity_Primitive_Hatch_Output(String aName, int aTier, String aDescription,
      ITexture[][][] aTextures, int aType) {
  ...
  public ITexture[] getTexture(IGregTechTileEntity aBaseMetaTileEntity, byte aSide, byte aFacing,
    ...
    if (aSide == aFacing) {
      ...
      return new ITexture[]{Textures.BlockIcons.casingTexturePages[3][23], TextureFactory.of(
              Textures.BlockIcons.OVERLAY_PIPE_OUT)};
    } else {
      if (this.mTypeHatch == 0) {
        return new ITexture[]{Textures.BlockIcons.casingTexturePages[1][53]};
        ...
        for (boolean temp = true; temp && mFluid != null; ) {
          ...
          FluidStack tDrained = aBaseMetaTileEntity
              .drain(ForgeDirection.getOrientation(aBaseMetaTileEntity.getFrontFacing()),
                  Math.max(1, mFluid.amount), false);
          if (tDrained != null) {
            int tFilledAmount = tTileEntity
                .fill(ForgeDirection.getOrientation(aBaseMetaTileEntity.getBackFacing()), tDrained,
                    false);
            if (tFilledAmount > 0) {
              temp = true;
  ...
  public void onScrewdriverRightClick(byte aSide, EntityPlayer aPlayer, float aX, float aY,
    ...
    switch (mMode) {
      ...
      case 8:
        ...
        GT_Utility.sendChatToPlayer(aPlayer, String
            .format("%s (%s)", trans("151.1", "Outputs items and 1 specific Fluid"), inBrackets));
        break;
      case 9:
        playerThatLockedfluid = aPlayer;
        if (mFluid == null) {
  ...
  public void onEmptyingContainerWhenEmpty() {
    if (this.lockedFluidName == null && this.mFluid != null) {
      ...
      GT_Utility.sendChatToPlayer(playerThatLockedfluid, String
          .format(trans("151.4", "Sucessfully locked Fluid to %s"), mFluid.getLocalizedName()));
    }
  }
}
