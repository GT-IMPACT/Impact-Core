...
public class GTMTE_HyperNaquadahGenerator extends GT_MetaTileEntity_MultiblockBase_EM implements
    IConstructable {
  ...
  private static final IStructureDefinition<GTMTE_HyperNaquadahGenerator> STRUCTURE_DEFINITION =
      StructureDefinition.<GTMTE_HyperNaquadahGenerator>builder()
          ...
          .addElement('C', ofChain(
              ofHatchAdder(GTMTE_HyperNaquadahGenerator::addClassicToMachineList, 410, sCaseCore2,
                  10),
        ...
        for (int i = 0; i < FUEL_NAME.length; i++) {
            RA.addFuel(FUEL_NAME[i],
                GT_Utility.getFluidForFilledItem(FUEL_NAME[i], true) == null ? GT_Utility
                    .getContainerItem(FUEL_NAME[i], true) : null, FUEL_PER_SECOND[i] * 256, 10);
        }
    }
  }
}
