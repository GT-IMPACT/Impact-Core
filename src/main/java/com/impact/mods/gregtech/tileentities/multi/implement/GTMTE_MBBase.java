...
public abstract class GTMTE_MBBase extends MetaTileEntity {
  ...
  public GTMTE_MBBase(int aID, String aName, String aNameRegional) {
  ...
  public GTMTE_MBBase(String aName) {
    super(aName, 2);
    GT_MetaTileEntity_MultiBlockBase.disableMaintenance = GregTech_API.sMachineFile
        .get(ConfigCategories.machineconfig, "MultiBlockMachines.disableMaintenance", false);
    this.damageFactorLow = GregTech_API.sMachineFile
  ...
  public void onPostTick(IGregTechTileEntity aBaseMetaTileEntity, long aTick) {
    if (aBaseMetaTileEntity.isServerSide()) {
          ...
          for (GT_MetaTileEntity_Hatch_Maintenance tHatch : mMaintenanceHatches) {
            if (isValidMetaTileEntity(tHatch)) {
              if (disableMaintenance) {
                ...
                if (tHatch.mAuto && !(mWrench && mScrewdriver && mSoftHammer && mHardHammer
            ...
            if (mMaxProgresstime > 0) {
              if (onRunningTick(mInventory[1])) {
                ...
                if (mMaxProgresstime > 0 && ++mProgresstime >= mMaxProgresstime) {
                  if (mOutputItems != null) {
                    for (ItemStack tStack : mOutputItems) {
                      if (tStack != null) {
                        try {
                          GT_Mod.achievements
                              .issueAchivementHatch(aBaseMetaTileEntity.getWorld()
                                  .getPlayerEntityByName(
                                      aBaseMetaTileEntity.getOwnerName()), tStack);
                  ...
                  mEfficiency = Math.max(0, Math.min(mEfficiency + mEfficiencyIncrease,
                      getMaxEfficiency(mInventory[1]) - ((getIdealStatus() - getRepairStatus())
                          * 1000)));
                  mOutputItems = null;
                  mProgresstime = 0;
                  mMaxProgresstime = 0;
      ...
      aBaseMetaTileEntity.setErrorDisplayID(
          (aBaseMetaTileEntity.getErrorDisplayID() & ~127) | (mWrench ? 0 : 1) | (mScrewdriver ? 0
              : 2) | (mSoftHammer ? 0 : 4) | (mHardHammer ? 0 : 8) | (mSolderingTool ? 0 : 16) | (
              mCrowbar ? 0 : 32) | (mMachine ? 0 : 64));
  ...
  public int getRepairStatus() {
    return (mWrench ? 1 : 0) + (mScrewdriver ? 1 : 0) + (mSoftHammer ? 1 : 0) + (mHardHammer ? 1
        : 0) + (mSolderingTool ? 1 : 0) + (mCrowbar ? 1 : 0);
  }
  ...
  public void explodeMultiblock() {
    ...
    GT_Log.exp.println(
        "MultiBlockExplosion at: " + this.getBaseMetaTileEntity().getXCoord() + " | " + this
            .getBaseMetaTileEntity().getYCoord() + " | " + this.getBaseMetaTileEntity().getZCoord()
            + " DIMID: " + this.getBaseMetaTileEntity().getWorld().provider.dimensionId + ".");
    ...
    for (MetaTileEntity tTileEntity : mInputBusses1) {
      tTileEntity.getBaseMetaTileEntity().doExplosion(V[8]);
    }
    for (MetaTileEntity tTileEntity : mOutputBusses) {
      tTileEntity.getBaseMetaTileEntity().doExplosion(V[8]);
    }
    for (MetaTileEntity tTileEntity : mOutputBusses1) {
      tTileEntity.getBaseMetaTileEntity().doExplosion(V[8]);
    }
    ...
    for (MetaTileEntity tTileEntity : mOutputHatches1) {
      tTileEntity.getBaseMetaTileEntity().doExplosion(V[8]);
    }
  ...
  protected void calculateOverclockedNessMulti(int aEUt, int aDuration, int mAmperage,
    ...
    if (mTier == 0) {
      ...
      while (tempEUt <= V[mTier - 1] * mAmperage) {
        ...
        xEUt = mMaxProgresstime == 0 ? xEUt >> 1
    ...
    while (outputSuccess && aStack.stackSize > 0) {
      ...
      for (GT_MetaTileEntity_Hatch_OutputBus tHatch : mOutputBusses) {
        if (!outputSuccess && isValidMetaTileEntity(tHatch)) {
          for (int i = tHatch.getSizeInventory() - 1; i >= 0 && !outputSuccess; i--) {
            if (tHatch.getBaseMetaTileEntity().addStackToSlot(i, single)) {
              outputSuccess = true;
            }
          }
        }
      ...
      for (GT_MetaTileEntity_Hatch_Output tHatch : mOutputHatches) {
        if (!outputSuccess && isValidMetaTileEntity(tHatch) && tHatch.outputsItems()) {
          if (tHatch.getBaseMetaTileEntity().addStackToSlot(1, single)) {
            outputSuccess = true;
          }
        }
      }
...
//        }
  ...
  public boolean addMaintenanceToMachineList(IGregTechTileEntity aTileEntity,
  ...
  public String[] getInfoData() {
    ...
    for (GT_MetaTileEntity_Hatch_Muffler tHatch : mMufflerHatches) {
      if (isValidMetaTileEntity(tHatch)) {
        mPollutionReduction = Math
            .max(tHatch.calculatePollutionReduction(100), mPollutionReduction);
      }
    }
    ...
    return new String[]{
        ...
        StatCollector.translateToLocal("GT5U.multiblock.Progress") + ": " + EnumChatFormatting.GREEN
            ...
            + EnumChatFormatting.YELLOW + Integer.toString(mMaxProgresstime / 20)
        ...
        EnumChatFormatting.YELLOW + VN[GT_Utility.getTier(getMaxInputVoltage())]
