...
public abstract class GTMTE_DrillerBase extends GT_MetaTileEntity_MultiBlockBase {
  ...
  private void initFields() {
    ...
    frameMeta =
        GregTech_API.METATILEENTITIES[frameId] != null ? GregTech_API.METATILEENTITIES[frameId]
  ...
  protected int tryLowerPipeState(boolean isSimulating) {
    ...
    if (!GT_Utility
        .setBlockByFakePlayer(getFakePlayer(getBaseMetaTileEntity()), xPipe, yHead - 1, zPipe,
            miningPipeTipBlock, 0, isSimulating)) {
      return 3;
    }
    if (!isSimulating) {
      if (yHead != yDrill) {
  ...
  protected int canLowerPipe() {
    ...
    if (yHead > 0
        && GT_Utility.getBlockHardnessAt(aBaseTile.getWorld(), xPipe, yHead - 1, zPipe) >= 0) {
      return
          GT_Utility.eraseBlockByFakePlayer(getFakePlayer(aBaseTile), xPipe, yHead - 1, zPipe, true)
              ? 0 : 2;
    }
    return 1;
  }
  ...
  protected boolean workingDownward(ItemStack aStack, int xDrill, int yDrill, int zDrill, int xPipe,
  ...
  protected boolean workingUpward(ItemStack aStack, int xDrill, int yDrill, int zDrill, int xPipe,
      int zPipe, int yHead, int oldYHead) {
    if (tryPickPipe()) {
      return true;
    } else {
...
//            }
    ...
    for (byte Y = -1; Y >= -2; Y--) {
      ...
      if (((thisController.getBlockOffset(offset1.x(), offset1.y(), offset1.z())
          == GregTech_API.sBlockMachines)
          && (thisController.getMetaIDOffset(offset1.x(), offset1.y(), offset1.z())
          == frameMeta))) {
        minCasingAmount--;
      } else {
        formationChecklist = false;
    ...
    for (byte Y = -1; Y >= -2; Y--) {
      ...
      if (((thisController.getBlockOffset(offset3.x(), offset3.y(), offset3.z())
          == GregTech_API.sBlockMachines)
          && (thisController.getMetaIDOffset(offset3.x(), offset3.y(), offset3.z())
          == frameMeta))) {
        minCasingAmount--;
      } else {
        formationChecklist = false;
    ...
    for (byte Y = -1; Y >= -2; Y--) {
      ...
      if (((thisController.getBlockOffset(offset4.x(), offset4.y(), offset4.z())
          == GregTech_API.sBlockMachines)
          && (thisController.getMetaIDOffset(offset4.x(), offset4.y(), offset4.z())
          == frameMeta))) {
        minCasingAmount--;
      } else {
        formationChecklist = false;
    ...
    for (byte Y = 1; Y <= 2; Y++) {
      ...
      if (((thisController.getBlockOffset(offset2.x(), offset2.y(), offset2.z())
          == GregTech_API.sBlockMachines)
          && (thisController.getMetaIDOffset(offset2.x(), offset2.y(), offset2.z())
          == frameMeta))) {
        minCasingAmount--;
      } else {
        formationChecklist = false;
