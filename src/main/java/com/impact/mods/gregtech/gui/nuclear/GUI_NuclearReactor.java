	...
	protected void drawGuiContainerForegroundLayer(int par1, int par2) {
		...
		if (this.mContainer != null) {
			...
			if ((mContainer).mDisplayErrorCode == 0) {
				...
				fontRendererObj
						.drawString(container.mInput < 0 ? "All Rods UP" : NumberFormat.getNumberInstance().format(container.mInput) + " L/t", 10, 105, 16448255);
				
				fontRendererObj
						.drawString("Output " + (container.isFastDecay ? "Hot Coolant:" : container.isMoxFuel ? "SH Steam:" : "Steam:"), 10, 120, 16448255);
				fontRendererObj
						.drawString(container.mOutput < 0 ? "All Rods UP" : (NumberFormat.getNumberInstance().format(container.mOutput) + " L/t"), 10, 130, 16448255);
