...
public class BlockAdvSolarPanel extends BlockContainer {
  ...
  public void registerBlockIcons(IIconRegister par1IconRegister) {
    ...
    for (byte Num = 0; Num <= 8; Num++) {
      for (byte i = 0; i <= 11; i++) {
        if (i == 1 || i == 7) {
          this.iconBuffer[Num][i] = par1IconRegister
              .registerIcon("impactsolarpanel:Panel" + Num + "_top");
        } else {
          this.iconBuffer[Num][i] = par1IconRegister
              .registerIcon("impactsolarpanel:Panel" + Num + "_side");
        }
      }
    }
  }
  ...
  public boolean onBlockActivated(World world, int i, int j, int k, EntityPlayer player, int s,
    ...
    if (tileentity instanceof IInventory) {
      ...
      for (int i = 0; i < inventory.getSizeInventory(); ++i) {
        ...
        if (item != null && item.stackSize > 0) {
          ...
          EntityItem entityItem = new EntityItem(world, (float) tileentity.xCoord + rx,
              (float) tileentity.yCoord + ry, (float) tileentity.zCoord + rz,
