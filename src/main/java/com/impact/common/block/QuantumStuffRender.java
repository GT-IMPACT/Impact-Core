...
public class QuantumStuffRender implements ISimpleBlockRenderingHandler {
  ...
  public void renderInventoryBlock(Block block, int metadata, int modelId, RenderBlocks renderer) {
    ...
    IIcon side = Block_QuantumStuff.stuff;
  ...
  public boolean renderWorldBlock(IBlockAccess world, int x, int y, int z, Block block, int modelId,
    ...
    IIcon side = Block_QuantumStuff.stuff;
    for (int i = 0; i < 6; i++) {
      float f = TecTech.RANDOM.nextFloat(), g = f - TecTech.RANDOM.nextFloat() / 2f, r =
          g - TecTech.RANDOM.nextFloat() / 3f - 0.25f;
      tes.setColorOpaque_F(TecTech.RANDOM.nextFloat(), 0, 0);
      float rotX = TecTech.RANDOM.nextFloat() * 2 * (float) Math.PI, rotY =
          TecTech.RANDOM.nextFloat() * 2 * (float) Math.PI, rotZ =
          TecTech.RANDOM.nextFloat() * 2 * (float) Math.PI;
  ...
  public int getRenderId() {
    return Block_QuantumStuff.renderID;
  }
}
