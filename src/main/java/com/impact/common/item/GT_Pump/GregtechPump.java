package com.impact.common.item.GT_Pump;
...
public class GregtechPump extends Item implements ISpecialElectricItem, IElectricItemManager,
    IFluidContainerItem {
  ...
  public GregtechPump(final String unlocalizedName, final CreativeTabs creativeTab,
      final short aOffset,
    ...
    if (GameRegistry.findItem(Refstrings.MODID, unlocalizedName) == null) {
  ...
  @Override
  public boolean onItemUse(ItemStack aStack, EntityPlayer aPlayer, World aWorld, int aX, int aY,
      int aZ, int a4,
  ...
  public void registerPumpType(final int aID, final String aPumpName, final int aEuMax,
      final int aTier) {
  ...
  public void registerItem(final int id, final String localizedName, final long euStorage,
      final int tier,
  ...
  @Override
  public void addInformation(final ItemStack aStack, final EntityPlayer aPlayer, List aList,
      final boolean aF3_H) {
    ...
    if (aOffsetMeta <= 3) {
      ...
      aList.add(
          EnumChatFormatting.BLUE + (f != null ? f.getLocalizedName() : "No Fluids Contained"));
      aList.add(EnumChatFormatting.BLUE + (f != null ? "" + f.amount : "" + 0) + "L" + " / "
          + getCapacity(aStack) + "L");
    }

    final Long[] tStats = this.getElectricStats(aStack);
    if (tStats != null) {
      if (tStats[3] > 0) {
        aList.add(EnumChatFormatting.AQUA + "Contains " + GT_Utility.formatNumbers(tStats[3])
            + " EU   Tier: "
    ...
    final ArrayList<IItemBehaviour<GregtechPump>> tList = this.mItemBehaviors
  ...
  public final double charge(final ItemStack aStack, final double aCharge, final int aTier,
    ...
    if ((tStats == null) || (tStats[2] > aTier)
        || !((tStats[3] == -1) || (tStats[3] == -3) || ((tStats[3] < 0) && (aCharge
        == Integer.MAX_VALUE)))
        || (aStack.stackSize != 1)) {
      return 0;
    }
    final long tChargeBefore = this.getRealCharge(aStack), tNewCharge = aCharge == Integer.MAX_VALUE
        ...
        : Math.min(Math.abs(tStats[0]),
            tChargeBefore + (aIgnoreTransferLimit ? (long) aCharge
  ...
  @Override
  public final boolean use(final ItemStack aStack, final double aAmount,
  ...
  public final void chargeFromArmor(final ItemStack aStack, final EntityLivingBase aPlayer) {
    ...
    for (int i = 1; i < 5; i++) {
      ...
      if (GT_ModHandler.isElectricItem(tArmor)) {
        ...
        if (tArmorItem.canProvideEnergy(tArmor) && (tArmorItem.getTier(tArmor) >= this
            .getTier(aStack))) {
  ...
  public final GregtechPump setBurnValue(final int aMetaValue, final int aValue) {
    ...
    if (aValue == 0) {
      ...
      this.mBurnValues
          .put((short) aMetaValue, aValue > Short.MAX_VALUE ? Short.MAX_VALUE : (short) aValue);
    }
    return this;
  }
   ...
   * @param aSpecialData   If this Item has a Fixed Charge, like a SingleUse Battery (if > 0). Use
   *                       -1 if you want to make this Battery chargeable (the use and canUse
   *                       Functions will still discharge if you just use this) Use -2 if you want
   *                       to make this Battery dischargeable. Use -3 if you want to make this
   *                       Battery charge/discharge-able.
   * @return the Item itself for convenience in constructing.
   */
  public final GregtechPump setElectricStats(final int aMetaValue, final long aMaxCharge,
  ...
  public final void registerIcons(final IIconRegister aIconRegister) {
    for (short i = 0, j = (short) this.mEnabledItems.length(); i < j; i++) {
      if (this.mEnabledItems.get(i)) {
        mIconMap.put((int) i, aIconRegister
            .registerIcon(Refstrings.MODID + ":" + (this.getUnlocalizedName() + "/" + i)));
      }
    }
  }
   ...
   * <p/>
   * Note: the boolean Behaviours sometimes won't be executed if another boolean Behaviour returned
   * true before.
   *
   ...
   * @return the Item itself for convenience in constructing.
   */
  public final GregtechPump addItemBehavior(final int aMetaValue,
      final IItemBehaviour<GregtechPump> aBehavior) {
   ...
   * @param aToolTip        The Default ToolTip of the created Item, you can also insert null for
   *                        having no ToolTip
   * @param //aFoodBehavior The Food Value of this Item. Can be null aswell. Just a convenience
   *                        thing.
   * @param aRandomData     The OreDict Names you want to give the Item. Also used for TC Aspects
   *                        and some other things.
   * @return An ItemStack containing the newly created Item.
   */
  @SuppressWarnings("unchecked")
  public final ItemStack addItem(final int aID, final String aEnglish, String aToolTip,
    ...
    if ((aID >= 0) && (aID < this.mItemAmount)) {
      ...
      GT_LanguageManager
          .addStringLocalization(this.getUnlocalizedName(rStack) + ".tooltip", aToolTip);
  ...
  public FluidStack getFluid(ItemStack container) {
    ...
    if (container.getTagCompound().hasKey("mInit") && container.getTagCompound()
        .getBoolean("mInit")) {
      String fluidname;
      Integer amount = 0;
  ...
  public int fill(ItemStack container, FluidStack resource, boolean doFill) {
    ...
    if (container.getTagCompound().hasKey("mInit") && container.getTagCompound()
      ...
      if (aStoredFluid == null) {
        ...
        toConsume = Utilits.getFluidStack(resource, amountToConsume);
        ...
        if (aStoredFluid.isFluidEqual(resource)) {
          ...
          toConsume = Utilits.getFluidStack(resource, (aStoredAmount + amountToConsume));
          if (toConsume != null && amountToConsume > 0) {
            storeFluid(container, toConsume);
            return amountToConsume;
          } else {
    ...
    if (container.getTagCompound().hasKey("mInit") && container.getTagCompound()
      ...
      if (maxDrain >= aStoredAmount) {
        ...
        if (amountRemaining == 0) {
          ...
          FluidStack newAmount = Utilits.getFluidStack(aStoredFluid, amountRemaining);
          FluidStack drained = Utilits.getFluidStack(aStoredFluid, maxDrain);
   ...
   */
  public boolean tryDrainTile(ItemStack aStack, World aWorld, EntityPlayer aPlayer, int aX, int aY,
      int aZ) {
    try {
      if (aWorld.isRemote || aStack == null) {
        ...
        if (!canUse(aStack, removal) && aTier > 0) {
          GT_Utility.sendChatToPlayer(aPlayer, "Not enough power.");
          return false;
        }
  ...
  public boolean drainIFluidTank(TileEntity tTileEntity, ItemStack aStack, World aWorld,
      EntityPlayer aPlayer, int aX,
      int aY, int aZ) {
    if (tTileEntity == null) {
      return false;
    }
    if ((tTileEntity instanceof IFluidTank || tTileEntity instanceof IFluidHandler)) {
      if (this.getFluid(aStack) == null || (this.getFluid(aStack) != null
          && this.getFluid(aStack).amount < this.getCapacity(aStack))) {
        ...
        if (aStored != null) {
          ...
          if (mAmountInserted > 0) {
            if (mAmountInserted == aStored.amount) {
              ...
              newStackRemainingInTank = Utilits
  ...
  public boolean drainTankGT(TileEntity tTileEntity, ItemStack aStack, World aWorld,
      EntityPlayer aPlayer, int aX,
      int aY, int aZ) {
    if (tTileEntity == null) {
      return false;
    ...
    if ((tTileEntity instanceof IGregTechTileEntity)) {
      if (((IGregTechTileEntity) tTileEntity).getTimer() < 50L) {
        return false;
      } else if ((!aWorld.isRemote) && (!((IGregTechTileEntity) tTileEntity)
          .isUseableByPlayer(aPlayer))) {
        return true;
      } else {
        if (this.getFluid(aStack) == null || (this.getFluid(aStack) != null
            && this.getFluid(aStack).amount < this.getCapacity(aStack))) {
  ...
  public boolean setStoredFluidOfVanillaTank(TileEntity aTileEntity, FluidStack aSetFluid) {
    ...
    if (aTileEntity == null) {
      ...
      if (aTileEntity instanceof IFluidTank) {
        ...
        return newStack.isFluidEqual(aSetFluid) && newStack.amount == aSetFluid.amount;
      } else {
