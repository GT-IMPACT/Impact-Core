...
public class NbtToJson {
	...
	public static GsonBuilder registerSmartNBTSerializer(GsonBuilder builder) {
	...
	public static Class<? extends NBTBase> getNBTTypeSmart(JsonElement element) {
		if (element.isJsonArray()) {
			...
			if (primitive.isString()) {
				...
				try {
					byte test = Byte.parseByte(primitive.getAsString());
					return NBTTagByte.class;
				} catch (Exception ignored){}
				
				try {
					short test = Short.parseShort(primitive.getAsString());
					return NBTTagShort.class;
				} catch (Exception ignored){}
				
				try {
					int test = Integer.parseInt(primitive.getAsString());
					return NBTTagInt.class;
				} catch (Exception ignored){}
				
				try {
					long test = Long.parseLong(primitive.getAsString());
					return NBTTagLong.class;
				} catch (Exception ignored){}

				try {
					float test = Float.parseFloat(primitive.getAsString());
					return NBTTagDouble.class;
				} catch (Exception ignored){}
				
				try {
					double test = Double.parseDouble(primitive.getAsString());
					return NBTTagDouble.class;
				} catch (Exception ignored){}
