...
{
  private LogHelper _mLogger = impact.Logger;
  ...
  public void LoadConfig()
  {
    ...
    if( !reload() )
    {
      ...
      impact.AddLoginError( "[CustomToolTips] Config file not loaded due errors" );
      InitSampleConfig();
    }
    _mInitialized = true;
  }
    ...
    if( !tPayload.isEmpty() )
    {
      ...
      if(pPlayer instanceof EntityPlayerMP)
      {
        for( CTTClientSyncMessage tMsg : tPendingMessages ) {
        	impact.NW.sendTo(tMsg, (EntityPlayerMP) pPlayer);
        }
      }
      else if( pPlayer == null )
      {
        for( CTTClientSyncMessage tMsg : tPendingMessages ) {
        	impact.NW.sendToAll(tMsg);
        }
      }
      else {
