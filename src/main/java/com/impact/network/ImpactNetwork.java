...
public class ImpactNetwork extends MessageToMessageCodec<FMLProxyPacket, ImpactPacket> {
	...
	public ImpactNetwork() {
		...
		this.mSubChannels = new ImpactPacket[]{
				...
				new ImpactPacketAerostat(), new ImpactPacketGuiTextField(), new ImpactPacketStringArray()
		};
	}
	...
	public void sendToAllAround(ImpactPacket aPacket, NetworkRegistry.TargetPoint aPosition) {
	...
	public void sendToAllAround(ImpactPacket aPacket, IGregTechTileEntity te, int range) {
		NetworkRegistry.TargetPoint aPosition =
				new NetworkRegistry.TargetPoint(te.getWorld().provider.dimensionId, te.getXCoord(), te.getYCoord(), te.getZCoord(), range);
		this.mChannel.get(Side.SERVER).attr(FMLOutboundHandler.FML_MESSAGETARGET).set(FMLOutboundHandler.OutboundTarget.ALLAROUNDPOINT);
		this.mChannel.get(Side.SERVER).attr(FMLOutboundHandler.FML_MESSAGETARGETARGS).set(aPosition);
		this.mChannel.get(Side.SERVER).writeAndFlush(aPacket);
	}
