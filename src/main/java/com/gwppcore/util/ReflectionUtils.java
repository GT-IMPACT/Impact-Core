...
public class ReflectionUtils {
	...
	private static void dumpClassInfo(Class<?> aClass) {
		...
		for (Method method : methods) {
			System.out.println(method.getName()+" | "+ SendUtils.getDataStringFromArray(method.getParameterTypes()));
		}
		for (Field f : fields) {
			System.out.println(f.getName());
		}
		for (Constructor<?> c : consts) {
			System.out.println(c.getName()+" | "+c.getParameterCount()+" | "+SendUtils.getDataStringFromArray(c.getParameterTypes()));
		}
	}
