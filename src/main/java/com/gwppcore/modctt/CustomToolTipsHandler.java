...
{
  private LogHelper _mLogger = gwppcore.Logger;
  ...
  public void LoadConfig()
  {
    ...
    if( !reload() )
    {
      ...
      gwppcore.AddLoginError( "[CustomToolTips] Config file not loaded due errors" );
      InitSampleConfig();
    }
    _mInitialized = true;
  }
    ...
    if( !tPayload.isEmpty() )
    {
      ...
      if(pPlayer instanceof EntityPlayerMP)
      {
        for( CTTClientSyncMessage tMsg : tPendingMessages ) {
        	gwppcore.NW.sendTo(tMsg, (EntityPlayerMP) pPlayer);
        }
      }
      else if( pPlayer == null )
      {
        for( CTTClientSyncMessage tMsg : tPendingMessages ) {
        	gwppcore.NW.sendToAll(tMsg);
        }
      }
      else {
