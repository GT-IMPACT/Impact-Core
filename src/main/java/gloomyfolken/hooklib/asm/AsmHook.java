...
/**
 * Класс, отвечающий за установку одного хука в один метод. Терминология: hook (хук) - вызов вашего
 * статического метода из стороннего кода (майнкрафта, форджа, других модов) targetMethod (целевой
 * метод) - метод, куда вставляется хук targetClass (целевой класс) - класс, где находится метод,
 * куда вставляется хук hookMethod (хук-метод) - ваш статический метод, который вызывается из
 * стороннего кода hookClass (класс с хуком) - класс, в котором содержится хук-метод
 */
public class AsmHook implements Cloneable, Comparable<AsmHook> {
  ...
  protected void createMethod(HookInjectorClassVisitor classVisitor) {
    ...
    MethodVisitor mv = classVisitor.visitMethod(Opcodes.ACC_PUBLIC,
        superMethod == null ? targetMethodName : superMethod.name, targetMethodDescription, null,
        null);
  ...
  private void injectSuperCall(HookInjectorMethodVisitor inj,
      ClassMetadataReader.MethodReference method) {
    int variableId = 0;
    for (int i = 0; i <= targetMethodParameters.size(); i++) {
      Type parameterType =
          i == 0 ? TypeHelper.getType(targetClassName) : targetMethodParameters.get(i - 1);
  ...
  private void injectInvokeStatic(HookInjectorMethodVisitor inj, int returnLocalId, String name,
      String desc) {
  ...
  public class Builder extends AsmHook {
    ...
    /**
     * --- ОБЯЗАТЕЛЬНО ВЫЗВАТЬ --- Определяет название класса, в который необходимо установить хук.
     *
     * @param className Название класса с указанием пакета, разделенное точками. Например:
    ...
    /**
     * --- ОБЯЗАТЕЛЬНО ВЫЗВАТЬ --- Определяет название метода, в который необходимо вставить хук.
    ...
    /**
     * --- ОБЯЗАТЕЛЬНО ВЫЗВАТЬ, ЕСЛИ У ЦЕЛЕВОГО МЕТОДА ЕСТЬ ПАРАМЕТРЫ --- Добавляет один или
     * несколько параметров к списку параметров целевого метода.
     * <p/>
     * Эти параметры используются, чтобы составить описание целевого метода. Чтобы однозначно
     * определить целевой метод, недостаточно только его названия - нужно ещё и описание.
     * <p/>
     * Примеры использования: import static gloomyfolken.hooklib.asm.TypeHelper.* //...
     * addTargetMethodParameters(Type.INT_TYPE) Type worldType = getType("net.minecraft.world.World")
    ...
    /**
     * Добавляет один или несколько параметров к списку параметров целевого метода. Обёртка над
     * addTargetMethodParameters(Type... parameterTypes), которая сама строит типы из названия.
     *
     * @param parameterTypeNames Названия классов параметров целевого метода. Например:
     *                           net.minecraft.world.World
     */
    ...
    /**
     * Изменяет тип, возвращаемый целевым методом. Вовращаемый тип используется, чтобы составить
     * описание целевого метода. Чтобы однозначно определить целевой метод, недостаточно только его
     * названия - нужно ещё и описание. По умолчанию хук применяется ко всем методам, подходящим по
     * названию и списку параметров.
     *
    ...
    /**
     * Изменяет тип, возвращаемый целевым методом. Обёртка над setTargetMethodReturnType(Type
     * returnType)
     *
    ...
    /**
     * --- ОБЯЗАТЕЛЬНО ВЫЗВАТЬ, ЕСЛИ НУЖЕН ХУК-МЕТОД, А НЕ ПРОСТО return SOME_CONSTANT ---
     * Определяет название хук-метода. ХУК-МЕТОД ДОЛЖЕН БЫТЬ СТАТИЧЕСКИМ, А ПРОВЕРКИ НА ЭТО НЕТ.
     * Будьте внимательны.
     *
    ...
    /**
     * --- ОБЯЗАТЕЛЬНО ВЫЗВАТЬ, ЕСЛИ У ХУК-МЕТОДА ЕСТЬ ПАРАМЕТРЫ --- Добавляет параметр в список
     * параметров хук-метода. В байткоде не сохраняются названия параметров. Вместо этого приходится
     * использовать их номера. Например, в классе EntityLivingBase есть метод
     * attackEntityFrom(DamageSource damageSource, float damage). В нём будут использоваться такие
     * номера параметров: 1 - damageSource 2 - damage ВАЖНЫЙ МОМЕНТ: LONG И DOUBLE "ЗАНИМАЮТ" ДВА
     * НОМЕРА. Теоретически, кроме параметров в хук-метод можно передать и локальные переменные, но
     * их номера сложнее посчитать. Например, в классе Entity есть метод setPosition(double x,
     * double y, double z). В нём будут такие номера параметров: 1 - x 2 - пропущено 3 - y 4 -
     * пропущено 5 - z 6 - пропущено
     * <p/>
     * Код этого метода таков: //... float f = ...; float f1 = ...; //... В таком случае у f будет
     * номер 7, а у f1 - 8.
     * <p/>
     ...
     * @throws IllegalStateException если не задано название хук-метода или класса, который его
     *                               содержит
     */
     ...
     *
     * @param parameterTypeName Название типа параметра хук-метода. Например: net.minecraft.world.World
     * @param variableId        ID значения, передаваемого в хук-метод
     */
    ...
    /**
     * Добавляет в список параметров хук-метода целевой класс и передает хук-методу this. Если
     * целевой метод static, то будет передано null.
     *
    ...
    /**
     * Добавляет в список параметров хук-метода тип, возвращаемый целевым методом и передает
     * хук-методу значение, которое вернёт return. Более формально, при вызове хук-метода указывает
     * в качестве этого параметра верхнее значение в стеке. На практике основное применение -
     * Например, есть такой код метода: int foo = bar(); return foo; Или такой: return bar()
     * <p/>
    ...
    /**
     * Задает условие, при котором после вызова хук-метода вызывается return. По умолчанию return не
     * вызывается вообще. Кроме того, этот метод изменяет тип возвращаемого значения хук-метода:
     * NEVER -> void ALWAYS -> void ON_TRUE -> boolean ON_NULL -> Object ON_NOT_NULL -> Object
     *
     * @param condition Условие выхода после вызова хук-метода
     * @throws IllegalArgumentException если condition == ON_TRUE, ON_NULL или ON_NOT_NULL, но не
     *                                  задан хук-метод.
     * @see ReturnCondition
     */
     ...
     * --- Задает значение, которое возвращается при вызове return после вызова хук-метода. Следует
     * вызывать после setReturnCondition. По умолчанию возвращается void. Кроме того, если value ==
     * ReturnValue.HOOK_RETURN_VALUE, то этот метод изменяет тип возвращаемого значения хук-метода
     * на тип, указанный в setTargetMethodReturnType()
     *
     * @param value возвращаемое значение
     * @throws IllegalStateException    если returnCondition == NEVER (т. е. если
     *                                  setReturnCondition() не вызывался). Нет смысла указывать
     *                                  возвращаемое значение, если return не вызывается.
     * @throws IllegalArgumentException если value == ReturnValue.HOOK_RETURN_VALUE, а тип
     *                                  возвращаемого значения целевого метода указан как void (или
     *                                  setTargetMethodReturnType ещё не вызывался). Нет смысла
     *                                  использовать значение, которое вернул хук-метод, если метод
     *                                  возвращает void.
     */
    public Builder setReturnValue(ReturnValue value) {
      ...
      if (value != ReturnValue.VOID && returnType == VOID_TYPE) {
        throw new IllegalArgumentException(
            "Target method return value is void, so it does not make sense to " +
                "return anything else.");
      }
      if (value == ReturnValue.VOID && returnType != VOID_TYPE) {
        throw new IllegalArgumentException(
            "Target method return value is not void, so it is impossible " +
                "to return VOID.");
      }
      if (value == ReturnValue.PRIMITIVE_CONSTANT && returnType != null && !isPrimitive(
          returnType)) {
        throw new IllegalArgumentException(
      ...
      if (value == ReturnValue.NULL && returnType != null && isPrimitive(returnType)) {
        throw new IllegalArgumentException(
            "Target method return value is a primitive, so it is impossible " +
                "to return NULL.");
      }
    ...
    /**
     * Возвращает тип возвращаемого значения хук-метода, если кому-то сложно "вычислить" его
     * самостоятельно.
     *
     ...
     * Следует вызывать после setReturnValue(ReturnValue.PRIMITIVE_CONSTANT) Задает константу,
     * которая будет возвращена при вызове return. Класс заданного объекта должен соответствовать
     * примитивному типу. Например, если целевой метод возвращает int, то в этот метод должен быть
     * передан объект класса Integer.
     *
     ...
     *                                  PRIMITIVE_CONSTANT
     * @throws IllegalArgumentException если класс объекта constant не является обёрткой для
     *                                  примитивного типа, который возвращает целевой метод.
     */
    public Builder setPrimitiveConstant(Object constant) {
      if (AsmHook.this.returnValue != ReturnValue.PRIMITIVE_CONSTANT) {
        throw new IllegalStateException(
            "Return value is not PRIMITIVE_CONSTANT, so it does not make sence" +
      ...
      if (returnType == BOOLEAN_TYPE && !(constant instanceof Boolean) ||
        ...
        throw new IllegalArgumentException(
            "Given object class does not math target method return type.");
      }
     ...
     * ANOTHER_METHOD_RETURN_VALUE --- Следует вызывать после setReturnValue(ReturnValue.ANOTHER_METHOD_RETURN_VALUE)
    ...
    /**
     * Задает фабрику, которая создаст инжектор для этого хука. Если говорить более человеческим
     * языком, то этот метод определяет, где будет вставлен хук: в начале метода, в конце или где-то
     * ещё. Если не создавать своих инжекторов, то можно использовать две фабрики:
    ...
    /**
     * Позволяет не только вставлять хуки в существующие методы, но и добавлять новые. Это может
     * понадобиться, когда нужно переопределить метод суперкласса. Если супер-метод найден, то тело
     * генерируемого метода представляет собой вызов супер-метода. Иначе это просто пустой метод или
     * return false/0/null в зависимости от возвращаемого типа.
     */
    ...
    /**
     * Позволяет объявить хук "обязательным" для запуска игры. В случае неудачи во время вставки
     * такого хука будет не просто выведено сообщение в лог, а крашнется игра.
     */
    ...
    public AsmHook build() {
      ...
      hook.targetMethodDescription = getMethodDesc(hook.targetMethodReturnType,
          hook.targetMethodParameters);
      ...
      if (hook.returnValue == ReturnValue.ANOTHER_METHOD_RETURN_VALUE) {
        hook.returnMethodDescription = getMethodDesc(hook.targetMethodReturnType,
            hook.hookMethodParameters);
      }
