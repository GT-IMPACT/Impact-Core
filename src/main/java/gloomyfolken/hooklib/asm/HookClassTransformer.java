...
public class HookClassTransformer {
    ...
    if (hooks != null) {
      ...
      try {
        ...
        ClassWriter cw = createClassWriter(
            java7 ? ClassWriter.COMPUTE_FRAMES : ClassWriter.COMPUTE_MAXS);
  ...
  /**
   * Создает ClassVisitor для списка хуков. Метод можно переопределить, если в ClassVisitor'e нужна
   * своя логика для проверки, является ли метод целевым (isTargetMethod())
   *
   ...
   */
  protected HookInjectorClassVisitor createInjectorClassVisitor(ClassWriter cw,
      List<AsmHook> hooks) {
    return new HookInjectorClassVisitor(this, cw, hooks);
  }

  /**
   * Создает ClassWriter для сохранения трансформированного класса. Метод можно переопределить, если
   * в ClassWriter'e нужна своя реализация метода getCommonSuperClass(). Стандартная реализация
   * работает для уже загруженных классов и для классов, .class файлы которых есть в classpath, но
   * они ещё не загружены. Во втором случае происходит загрузка (но не инициализация) классов. Если
   * загрузка классов является проблемой, то можно воспользоваться SafeClassWriter.
   *
